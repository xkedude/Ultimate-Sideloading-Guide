Introduction

Hello, Welcome to my guide on how to sideload on Apple Devices using a method I came across to a while back. This sideloading method has worked for long periods of time, without any app limits and without a pc. I eventually found some small let-downs such as the go-to DNS at the time was and is still made by some random Vietnamese guy with no open-sourced data which naturally has some privacy concerns. ESign also had undesirable analytics which are now blocked by my DNS.

After a few months of using both of these services I decided to reverse-engineer the DNS and how the enterprise signed ESign was downloaded over the web with the click of a button. With enabling better preventions against revokes unlike other people who keep on using other people's DNS and put no effort in themselves with bad instructions to say the least.

Some of the advantages with using this method

Free!
No PC!
No App Limits!
On Device Signing!
No Privacy Concerns!
Easy to Setup and use!
Working ChatGPT!
Takes 10 Minutes to Setup! The DNS is compatible with other services such as Scarlet, Feather and any other enterprise signing app, More info here
Without further ado, welcome to my Ultimate Sideloading Guide, if you have any issues, requests or ideas please make an issue here on GitHub.

Scarlet, Feather and other sideloaders

The DNS is compatible with all services as long as they rely on enterprise certificates to install.

Feather is also available to download and the steps should be similar as ESign's.

Follow Part 1 and 2 of my guide to install the DNS blocker for these services.

You do not need to follow Part 3, 4 and 5 as these install and setup ESign itself.

How does this method work?

By using leaked business enterprise certificates, We can sign apps using their credentials which means Apple lets us install the apps without any hassle, however after a while Apple revokes the certificate, this is where the DNS Blocker comes in, the DNS blocker blocks Apple servers from contacting your device about the expired/revoked/blacklisted certificate which in turn lets you use the apps for however long you'd like. The DNS also blocks ESign analytics.

Before we start

Make sure you have not been using services such as Scarlet and Feather before as the certificates used to install ESign on your device might be revoked!
If you are blacklisted or revoked from Apple for already using services such as Scarlet and Feather you will need to follow this guide
This method will work as long as you read and do all of the steps required
Compatibility

This should work for all iOS and iPadOS devices on the latest versions

Beta versions will work but may become revoked around the 1-2 week mark. It is better to be on a release version!

Versions that might not be supported are below iOS 12, iPadOS 13

Part [1/5] Making your DNS

Go to this website and make your own account

Go to the Denylist tab and add the following domains:

vpp.itunes.apple.com

appattest.apple.com

certs.apple.com

crl.apple.com

valid.apple.com

ocsp2.apple.com

ocsp.apple.com

Go to the Allowlist tab and add the following domains:

app.localhost.direct

register.appattest.apple.com

Go into the Privcy tab and click on the cross next to the NextDNS Ads and Trackers

Continue onto Part 2 to finish the DNS installation

Part [2/5] Installing the DNS

Go to the Setup tab, find and click on the hyperlinked apple.nextdns.io which should redirect you to the NextDNS Profile website
Select the profile you edited
Click on More Options
Then Enable the Trust NextDNS Root CA and Bootstrap IPs toggle
Click on Download, and the profile should be imported into settings automatically
Now, open settings and go to General > VPN & Device Management and install the downloaded profile
The DNS should now automatically be enabled, Do not change the DNS settings to automatic otherwise your apps will get revoked!
Now, go back and then go to General > About > Certificate Trust Settings
Enable Full Trust for the NextDNS Root Certificate, if Full Trust does not show you cannot use VPNs at all
Continue onto Part 3 if you want to install ESign
Part [3/5] Installing ESign

Go down to Downloads and click on the link, then click on Downloads, then ESign and choose any link Some users report that the Henan, Sunshine and VietnamPost links work better than others.
Click on install once the popup comes up
Open settings and go to General > VPN & Device Management > (should show the ESign app under the link you picked back in Step 1.)
Click on Trust at the top then click on allow, after this your device might need to be restarted
If your device was restarted, swipe up and click on install profile
ESign should now be on your home screen, fully functional
Continue onto Part 4 to setup ESign
Part [4/5] Setting up ESign

Open ESign and accept the agreements
Go into Settings and click on Get Device UDID
Select allow and then you should be redirected to settings
After opening settings go to General > VPN & Device Management
Install the Query Device UDID profile
You should be redirected back to ESign
Now go down to Downloads, click the link and then go into certificates, download the ESign certificates, and the Certificates should be downloaded
Extract the ESign-Certificates.zip and select the one that corresponds to the ESign app in VPN & Device Management
Go into ESign Settings and click on Import Resources
Find and select the certificate you selected back in Step 8 and click on Import
Now go over to AppStore in ESign and click on App Source
Click on the plus icon and paste in the sources from here, click on add all
You now have setup ESign with all sources ready to install apps
Continue onto Part 5
Part [5/5] Using ESign

Download any app using the AppStore in ESign or you can import your own .ipa using Import Resource in which case you can skip to step 4
Check the download status in the Download tab if downloading from the ESign AppStore
Click on the downloaded IPA and select Import App Library
Your app should now be visible in the Unsigned Apps tab, select unsigned or signed by selecting the bar on top of ESign
Click on the app and select Signature
You can change your app and edit it here
Select Signature and ESign should sign the app using the certificate you selected before
There might be red text saying this certificate is revoked, this is normal! Click on the blue button saying Install, then click Install on the prompt again
The app should be on your homescreen now
Open settings and go to General > VPN & Device Management
Select the app you installed and click on Trust
Click on Allow, after this your device might need to be restarted
If your device was restarted, swipe up and click on Install Profile
The app should now be on your home screen, fully functional
Now read the Post Install Notes
Post Install Notes

You must never disable the DNS!
Don't exit ESign while the app is being signed!
Don't install a lot of apps at once!
Don't install Safari extensions!
Go into Airplane Mode if you're switching DNS
Don't use Cowabunga Lite otherwise the DNS will disappear! There is a workaround, i'll make a guide soon!
Don't use Nugget otherwise the DNS will disappear! There is a workaround, i'll make a guide soon!
NextDNS has a limit of 300000 queries per month! I recommend heavily that you do not share your profile as you might reach this limit, I also recommend that you make 2 or more accounts with NextDNS and switch if your monthly quota is met for 1 of those profiles!
Not following the above guidelines will make your apps revoked!

Troubleshooting

Unable to Install? Integrity could not be verified?

You need to follow this guide

Downloads

All Downloads!

ESign Sources

My sources for ESign

source[]
Questions/Answers

Q - Won't a VPN bypass the DNS and revoke my apps?

A - It probably will but the DNS settings should theoretically prevent it. I do not recommend using a VPN. If you really want to use a VPN, Use this guide.

Q - Will a restart bypass the DNS and revoke my apps?

A - Simple answer, No.

Q - Will services such as AltStore, Sideloadly and Sidestore affect this guide at all?

A - No, Those listed services use developer signing instead of Enterprise signing so they are fine to use alongside! You might need to edit SideStore's WireGuard and edit the DNS inside to match your NextDNS settings in this guide.

Miscellaneous Guides

Using a VPN

So you want to use a VPN with the DNS blocker, follow the steps below and you should be able to use your VPN normally and on-demand, I still do not recommend using a VPN with the DNS, You should know how to configure your VPN's DNS server yourself as there are different steps for each VPN app.

Method 1

Find the DNS under the subheading Linked IP
Use either one or both if you can, and replace them in your VPN's DNS settings.
Revoked?

So you've gotten yourself revoked, either from the DNS (which seems very unlikely) or from another creator's DNS. Follow the methods listed below and you should be unrevoked.

Method 1 is very easy to do and has a very high success rate of removing revokes Method 2 is a hit or miss, but should work most of the time Method 3 will completely reset your device

You must not use your backups with Method 3 otherwise you will get revoked again.

Method 1

Remove the DNS and uninstall ESign
Back up your device just in case, here is a guide
Download and install 3uTools
Connect your device and go to the Smart Flash Tab at the top
Download the iOS version you currently have or higher
After the download, select the firmware and toggle the Retain User's Data on
Then select flash! Make sure to not disconnect your phone at all
Then follow Part 1 and 2 of the guide to prevent revokes in the future
Method 2

Remove the DNS and uninstall ESign
Back up your device, here is a guide
After backing up, reset your device, here is a guide
Now if you have an iCloud backup you should be prompted to restore from your backup after logging in to your Apple ID, if you have a local backup all you need to do is plug your phone in, go on either Finder for macOS or iTunes for Windows and click on restore backup from your device's menu
Then follow Part 1 and 2 of the guide to prevent revokes in the future
Method 3

Remove the DNS and uninstall ESign
Follow this guide
Then follow Part 1 and 2 of the guide to prevent revokes in the future
Using Nugget or Cowabungs with the DNS blocker

Profiles disappear when using Nugget and Cowabunga. This can easily be fixed but is a bit annoying as it requires a PC.

Fix for macOS:

Only the macOS version of Nugget is compatible with this guide (only on older ios/devices its needed). Not the mobile app itself! You must always follow this guide when wanting to use Nugget and Cowabunga!

Part [1/2] Using Nugget/Cowabunga

Forget all WiFi networks and disable Cellular Networks and put your device in Airplane Mode.
Remove every Configuration Profile you have, which are your DNS profiles. Don't delete your installed apps.
Now you can use Nugget or Cowabunga as you please.
Continue onto the Part 2
Part [2/2] Installing the DNS

Go to this website on your macOS device.
Go to the Setup tab, find and click on the hyperlinked apple.nextdns.io which should redirect you to the NextDNS Profile website
Select the profile you made and click on More Options
Then Enable the Trust NextDNS Root CA and Bootstrap IPs toggle
Click on Download, and the profile should be downloaded into your Downloads folder
Download Apple Configurator from the Mac App Store
Make sure your device is connected to your macOS device
Input your password on your device if it requires it
Find your device, right click and select Add > Profiles
Select your profile and then it should be installed and activated automatically
